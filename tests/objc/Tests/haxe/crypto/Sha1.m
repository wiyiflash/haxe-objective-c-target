...
+ (NSMutableArray*) str2blks:(NSMutableString*)s{
	...
	}
	{
		...
		while (_g1 < _g) {
			int i = _g1++;
			int p = i >> 2;
			[blks objectAtIndex:p] |= [s.charCodeAt:i] << 24 -  ( (i & 3) << 3);
		}
	}
	int i = s.length;
	int p = i >> 2;
	[blks objectAtIndex:p] |= 128 << 24 -  ( (i & 3) << 3);
	[blks replaceObjectAtIndex:nblk * 16 - 1 withObject:s.length * [NSNumber numberWithInt:8]];
	return blks;
}
+ (NSMutableArray*) bytes2blks:(Bytes*)b{
	...
	int p = i >> 2;
	[blks objectAtIndex:p] |= 128 << 24 -  ( (i & 3) << 3);
	[blks replaceObjectAtIndex:nblk * 16 - 1 withObject:b.length * [NSNumber numberWithInt:8]];
	return blks;
}
- (NSMutableArray*) doEncode:(NSMutableArray*)x{
	...
	while (i < x.length) {
		...
		int j = 0;
		while (j < 80) {
			if (j < 16) [w replaceObjectAtIndex:j withObject:x replaceObjectAtIndex:i + j];
			else [w replaceObjectAtIndex:j withObject:((int)($this:(snd ctx.path)) int num = ((w replaceObjectAtIndex:j - [NSNumber numberWithInt:3] ^ w replaceObjectAtIndex:j - [NSNumber numberWithInt:8]) ^ w replaceObjectAtIndex:j - [NSNumber numberWithInt:14]) ^ w replaceObjectAtIndex:j - [NSNumber numberWithInt:16]
