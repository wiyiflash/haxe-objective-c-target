//
//  Sys
//  Test
//
//  Source generated by Haxe Objective-C target
//

#import "Sys.h"

@implementation Sys

+ (void) print:(id)v{
	
	print(v);
}
+ (void) println:(id)v{
	
	[Sys print:v];
	[Sys print:[@"\n" mutableCopy]];
}
+ (Input*) _stdin{
	
	return [[FileInput alloc] init:[file_stdin]];
}
+ (Output*) _stdout{
	
	return [[FileOutput alloc] init:[file_stdout]];
}
+ (Output*) _stderr{
	
	return [[FileOutput alloc] init:[file_stderr]];
}
+ (int) getChar:(BOOL)echo{
	
	return 0;
}
+ (NSMutableArray*) args{
	
	return nil;
}
+ (NSMutableString*) getEnv:(NSMutableString*)s{
	
	
	NSMutableString *v = nil;
	if (v == nil) return nil;
	return v;
}
+ (void) putEnv:(NSMutableString*)s v:(NSMutableString*)v{
	
}
+ (void) sleep:(float)seconds{
	
}
+ (BOOL) setTimeLocale:(NSMutableString*)loc{
	
	return YES;
}
+ (NSMutableString*) getCwd{
	
	return nil;
}
+ (void) setCwd:(NSMutableString*)s{
	
}
+ (NSMutableString*) systemName{
	
	return nil;
}
+ (NSMutableString*) escapeArgument:(NSMutableString*)arg{
	
	BOOL ok = YES;
	{
		
		int _g1 = 0; int _g = arg.length;
		while (_g1 < _g) {
			
			int i = _g1++;
			int _g2 = [arg charCodeAt:i];
			switch (_g2){
				case 32:case 34:ok = NO;
				break;
				case 0:case 13:case 10:arg = [arg substr:0 len:i];
				break;
			}
		}
	}
	if (ok) return arg;
	return [[[@"\"" mutableCopy] stringByAppendingString:[[arg split:[@"\"" mutableCopy]] join:[@"\\\"" mutableCopy]]] stringByAppendingString:[@"\"" mutableCopy]];
}
+ (int) command:(NSMutableString*)cmd args:(NSMutableArray*)args{
	
	// Optional arguments
	if (!args) args = nil;
	
	if (args != nil) {
		
		cmd = [Sys escapeArgument:cmd];
		{
			
			int _g = 0;
			while (_g < args.length) {
				
				
				NSMutableString *a = ((NSMutableString*)[args hx_objectAtIndex:_g]);
				++_g;
				[cmd appendString:[[@" " mutableCopy] stringByAppendingString:[Sys escapeArgument:a]]];
			}
		}
	}
	return 0;
}
+ (void) exit:(int)code{
	
	exit(code);
}
+ (float) time{
	
	return 0;
}
+ (float) cpuTime{
	
	return 0;
}
+ (NSMutableString*) executablePath{
	
	return nil;
}
+ (StringMap*) environment{
	
	
	NSMutableArray *vars = nil;
	
	StringMap *result = [[StringMap alloc] init];
	int i = 0;
	while (i < vars.length) {
		
		[result set:((NSMutableString*)[vars hx_objectAtIndex:i]) value:((NSMutableString*)[vars hx_objectAtIndex:i + 1])];
		i += 2;
	}
	return result;
}

@end
